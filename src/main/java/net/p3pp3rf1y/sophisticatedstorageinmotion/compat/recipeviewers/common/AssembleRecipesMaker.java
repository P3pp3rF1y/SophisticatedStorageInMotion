	...
	private static <T extends MovingStorageFromStorageRecipe, U extends PropertyBasedSubtypeInterpreter> MovingStorageTierUpgradeDisplayRecipe createDisplayRecipe(
		...
		for (Optional<Ingredient> ingredient : ingredients) {
			...
			if (!ingredientItems.isEmpty() && ingredientItems.getFirst().getItem() instanceof StorageBlockItem) {
				...
				ingredientsTemplate.add(i, RecipeViewerIngredients.empty());
			} else {
				ingredientsTemplate.add(i, ingredient.orElseGet(RecipeViewerIngredients::empty));
		...
		ResourceLocation id = recipeHolder.id().location().withPath(path -> "assemble_moving_storage_grouped/" + path);
