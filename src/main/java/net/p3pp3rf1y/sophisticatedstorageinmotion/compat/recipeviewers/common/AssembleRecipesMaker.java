...
public class AssembleRecipesMaker {
	...
	private static <T extends MovingStorageFromStorageRecipe, U extends PropertyBasedSubtypeInterpreter> MovingStorageTierUpgradeDisplayRecipe createDisplayRecipe(RecipeHolder<T> recipeHolder,
		...
		List<Optional<Ingredient>> ingredients = new ArrayList<>(RecipeViewerRecipeHelper.getIngredients(recipe));
		...
		RecipeHolder<CraftingRecipe> displayRecipeHolder = new RecipeHolder<>(recipeHolder.id(), new ShapelessRecipe(new Recipe.CommonInfo(true), new CraftingRecipe.CraftingBookInfo(CraftingBookCategory.MISC, ""), ItemStackTemplate.fromNonEmptyStack(ClientRecipeHelper.getResultItem(recipe)), ingredientsTemplate));
