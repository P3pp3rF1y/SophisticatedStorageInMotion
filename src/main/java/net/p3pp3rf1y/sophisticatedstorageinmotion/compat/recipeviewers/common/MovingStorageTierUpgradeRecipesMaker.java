...
public class MovingStorageTierUpgradeRecipesMaker {
	...
	private static List<ItemStack> getIngredientItems(Optional<Ingredient> ingredient) {
		return ingredient.map(value -> {
			if (value.getCustomIngredient() instanceof MovingStorageIngredient movingStorageIngredient) {
				return movingStorageIngredient.getMatchingStacks();
			}
			return value.items().map(ItemStack::new).toList();
		}).orElse(List.of());
	}
